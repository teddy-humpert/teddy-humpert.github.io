...
// div takes multi and flips it on its head -- fixed
function problemPopulator() {
  let problemArray = [];
  let int1 = getRandomNumber(10); // random bw 0-9
  let int2 = getRandomNumber(10); // random bw 0-9
  let dividend = int1 * int2; // correct answer
  let answer = int1;
  let problemString = dividend + " / " + int2; // "5 X 9"
  //change innertext of problem?
  problemArray.push(dividend);
...
function init() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    let problem = { equation: problemArray[0] + ' / ' + problemArray[1], answer: problemArray[2], isCorrect: false, answers: answerPool };
...
function populateAnswers(answer) {
  ...
  while (answers.length < 4) { // populates rest of the array
    let wrongAnswer = getRandomNumber(10); // random number between 0*0 and 9*9
...
function getRandomNumber(max) { // need to clean this up.
